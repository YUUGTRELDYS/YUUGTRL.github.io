...
function YUUGTRL:CreateToggle(parent, text, default, callback, position, size, colors)
    return self:CreateButtonToggle(parent, text, default, callback, position, size, colors)
end
...
function YUUGTRL:CreateDropdown(parent, label, options, default, callback, position, size)
    if not parent then return end
    ...
    self:CreateLabel(frame, label or "", UDim2.new(0, 10 * scale, 0, 5 * scale), UDim2.new(1, -10 * scale, 0, 20 * scale))
    
    local dropdownBtn = self:CreateButton(frame, selected, nil, currentTheme.ButtonColor, UDim2.new(0, 10 * scale, 0, 30 * scale), UDim2.new(1, -20 * scale, 0, 30 * scale))
    dropdownBtn.TextXAlignment = Enum.TextXAlignment.Left
    dropdownBtn.Text = selected
    
    local dropdownFrame = self:CreateFrame(parent, UDim2.new(0, 200 * scale, 0, 0), UDim2.new(position.X.Scale, position.X.Offset, position.Y.Scale, position.Y.Offset + (size and size.Y.Offset or 50 * scale)), currentTheme.FrameColor, 8 * scale)
    ...
    scrollFrame.CanvasSize = UDim2.new(0, 0, 0, #options * 35 * scale)
    ...
    for _, option in ipairs(options) do
        local btn = self:CreateButton(scrollFrame, option, nil, currentTheme.ButtonColor, nil, UDim2.new(1, -5 * scale, 0, 35 * scale))
    ...
    dropdownBtn.MouseButton1Click:Connect(function()
        ...
        if isOpen then
            ...
            dropdownFrame.Position = UDim2.new(0, pos.X, 0, pos.Y + frame.AbsoluteSize.Y)
            dropdownFrame.Size = UDim2.new(0, frame.AbsoluteSize.X, 0, math.min(#options * 38 * scale, 200 * scale))
        end
    end)
...
function YUUGTRL:CreateColorpicker(parent, label, default, callback, position, size)
    if not parent then return end
    
    local selectedColor = default or currentTheme.AccentColor
    
    local frame = self:CreateFrame(parent, size or UDim2.new(0, 200 * scale, 0, 50 * scale), position, currentTheme.FrameColor, 8 * scale)
    
    self:CreateLabel(frame, label or "", UDim2.new(0, 10 * scale, 0, 5 * scale), UDim2.new(1, -10 * scale, 0, 20 * scale))
    
    local colorPreview = self:CreateFrame(frame, UDim2.new(0, 40 * scale, 0, 30 * scale), UDim2.new(0, 10 * scale, 0, 30 * scale), selectedColor, 6 * scale)
    colorPreview.InputChanged:Connect(function()
        colorPreview.BackgroundColor3 = selectedColor
    end)
    
    local valueLabel = self:CreateLabel(frame, string.format("RGB: %d, %d, %d", selectedColor.R * 255, selectedColor.G * 255, selectedColor.B * 255), UDim2.new(0, 60 * scale, 0, 30 * scale), UDim2.new(1, -70 * scale, 0, 30 * scale))
    valueLabel.TextXAlignment = Enum.TextXAlignment.Left
    valueLabel.TextSize = 10 * scale
    
    local pickerFrame = self:CreateFrame(parent, UDim2.new(0, 200 * scale, 0, 180 * scale), UDim2.new(0, 0, 0, 0), currentTheme.FrameColor, 8 * scale)
    pickerFrame.Visible = false
    pickerFrame.ZIndex = 10
    
    local hueSlider = self:CreateFrame(pickerFrame, UDim2.new(1, -20 * scale, 0, 15 * scale), UDim2.new(0, 10 * scale, 0, 10 * scale), Color3.fromRGB(255, 0, 0), 4 * scale)
    
    local hueFill = Instance.new("UIGradient")
    ...
    local satValFrame = self:CreateFrame(pickerFrame, UDim2.new(0, 180 * scale, 0, 100 * scale), UDim2.new(0, 10 * scale, 0, 35 * scale), Color3.fromRGB(255, 0, 0), 4 * scale)
    ...
    satValGradient2.Rotation = 90
    satValGradient2.Parent = satValFrame
    
    local okBtn = self:CreateButton(pickerFrame, "OK", nil, currentTheme.ButtonColor, UDim2.new(1, -20 * scale, 1, -40 * scale), UDim2.new(0, 80 * scale, 0, 30 * scale))
    
    colorPreview.MouseButton1Click:Connect(function()
        pickerFrame.Visible = true
        local pos = frame.AbsolutePosition
        pickerFrame.Position = UDim2.new(0, pos.X, 0, pos.Y + frame.AbsoluteSize.Y)
    end)
    
    okBtn.MouseButton1Click:Connect(function()
        pickerFrame.Visible = false
        if callback then pcall(callback, selectedColor) end
        valueLabel.Text = string.format("RGB: %d, %d, %d", selectedColor.R * 255, selectedColor.G * 255, selectedColor.B * 255)
        colorPreview.BackgroundColor3 = selectedColor
    end)
...
function YUUGTRL:CreateTextbox(parent, label, default, callback, position, size)
    if not parent then return end
    ...
    textBox.Size = UDim2.new(1, -20 * scale, 0, 30 * scale)
    textBox.Position = UDim2.new(0, 10 * scale, 0, 30 * scale)
    textBox.BackgroundColor3 = currentTheme.InputColor
...
function YUUGTRL:CreateTab(parent, text, position, size)
    if not parent then return end
    
    local frame = self:CreateFrame(parent, size or UDim2.new(0, 200 * scale, 0, 35 * scale), position, currentTheme.FrameColor, 8 * scale)
    
    local label = self:CreateLabel(frame, text, UDim2.new(0, 0, 0, 0), UDim2.new(1, 0, 1, 0), currentTheme.TextColor)
    label.TextXAlignment = Enum.TextXAlignment.Center
    
    frame.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            for _, child in pairs(parent:GetChildren()) do
                if child:IsA("Frame") and child ~= parent and child:FindFirstChildOfClass("UIGradient") then
                    child.BackgroundColor3 = currentTheme.FrameColor
                end
            end
            frame.BackgroundColor3 = currentTheme.AccentColor
        end
    end)
    ...
    function tabObject:SetText(newText)
        label.Text = newText
    end
...
function YUUGTRL:CreateSection(parent, title, position, size)
    if not parent then return end
    
    local frame = self:CreateFrame(parent, size or UDim2.new(1, -10 * scale, 0, 40 * scale), position, currentTheme.FrameColor, 8 * scale)
    
    local titleLabel = self:CreateLabel(frame, title, UDim2.new(0, 10 * scale, 0, 5 * scale), UDim2.new(1, -20 * scale, 0, 25 * scale), currentTheme.TextColor)
    titleLabel.TextSize = 16 * scale
    titleLabel.Font = Enum.Font.GothamBold
    
    local line = self:CreateFrame(frame, UDim2.new(1, -20 * scale, 0, 2 * scale), UDim2.new(0, 10 * scale, 0, 30 * scale), currentTheme.AccentColor, 1 * scale)
    ...
    function sectionObject:SetTitle(newTitle)
...
function YUUGTRL:CreateWindow(title, size, position, options)
    ...
    function window:CreateToggle(text, default, callback, position, size, colors, translationKey)
        return window:CreateButtonToggle(text, default, callback, position, size, colors, translationKey)
    end

    function window:CreateDropdown(label, options, default, callback, position, size, translationKey)
        ...
        return YUUGTRL:CreateDropdown(self.Main, label, options, default, callback, dropPos, dropSize)
    end

    function window:CreateColorpicker(label, default, callback, position, size, translationKey)
        local colorPos = position and UDim2.new(position.X.Scale, position.X.Offset * self.scale, position.Y.Scale, position.Y.Offset * self.scale) or nil
        local colorSize = size and UDim2.new(size.X.Scale, size.X.Offset * self.scale, size.Y.Scale, size.Y.Offset * self.scale) or nil
        return YUUGTRL:CreateColorpicker(self.Main, label, default, callback, colorPos, colorSize)
    end

    function window:CreateTextbox(label, default, callback, position, size, translationKey)
        local textPos = position and UDim2.new(position.X.Scale, position.X.Offset * self.scale, position.Y.Scale, position.Y.Offset * self.scale) or nil
        local textSize = size and UDim2.new(size.X.Scale, size.X.Offset * self.scale, size.Y.Scale, size.Y.Offset * self.scale) or nil
        return YUUGTRL:CreateTextbox(self.Main, label, default, callback, textPos, textSize)
    end

    function window:CreateTab(text, position, size, translationKey)
        local tabPos = position and UDim2.new(position.X.Scale, position.X.Offset * self.scale, position.Y.Scale, position.Y.Offset * self.scale) or nil
        local tabSize = size and UDim2.new(size.X.Scale, size.X.Offset * self.scale, size.Y.Scale, size.Y.Offset * self.scale) or nil
        return YUUGTRL:CreateTab(self.Main, text, tabPos, tabSize)
    end

    function window:CreateSection(title, position, size, translationKey)
        local sectionPos = position and UDim2.new(position.X.Scale, position.X.Offset * self.scale, position.Y.Scale, position.Y.Offset * self.scale) or nil
        local sectionSize = size and UDim2.new(size.X.Scale, size.X.Offset * self.scale, size.Y.Scale, size.Y.Offset * self.scale) or nil
        return YUUGTRL:CreateSection(self.Main, title, sectionPos, sectionSize)
    end
